...
const add = (a, b) => {
	...
	let exp
	if (a.exp < b.exp) {
		man = a.man + (b.man * 10n ** (b.exp - a.exp))
		exp = a.exp
	} else {
		man = (a.man * 10n ** (a.exp - b.exp)) + b.man
		exp = b.exp
	}
...
const fromString = (s) => {
	const match = s.match(/^(?<whole>-?\d+)(?:\.(?<decimal>\d+))?(?:[eE](?<exp>-?\d+))?$/u)
	if (!match) { return NAN }
